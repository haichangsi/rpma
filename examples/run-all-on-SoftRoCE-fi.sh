...
if [ x"$VLD_TEST" == x"--valgrind" ]; then
	...
	VLD_SUPP="${VLD_SUPP} --suppressions=${VLD_SUPP_PATH}/memcheck-FI.supp"
...
function run_example() {
	...
	echo "$SRV $VLD_SCMD $DIR/server $IP_ADDRESS $PORT"
	eval $SRV $VLD_SCMD $DIR/server $IP_ADDRESS $PORT &
	sleep 1
	...
	06-multiple-connections)
		for SEED in 8 9 11 12; do
			...
			echo "$CLT $VLD_CCMD $DIR/client $IP_ADDRESS $PORT $SEED"
			eval $CLT $VLD_CCMD $DIR/client $IP_ADDRESS $PORT $SEED
			RV=$?
			[ $RV -ne 0 ] && break
		done
		;;
	07-atomic-write)
		echo "Starting the client ..."
		echo $CLT $VLD_CCMD $DIR/client $IP_ADDRESS $PORT "1st_word" "2nd_word" "3rd_word"
		eval $CLT $VLD_CCMD $DIR/client $IP_ADDRESS $PORT "1st_word" "2nd_word" "3rd_word"
		RV=$?
		;;
	08-messages-ping-pong)
		...
		ROUNDS=3
		echo "Starting the client ..."
		echo "$CLT $VLD_CCMD $DIR/client $IP_ADDRESS $PORT $SEED $ROUNDS"
		eval $CLT $VLD_CCMD $DIR/client $IP_ADDRESS $PORT $SEED $ROUNDS
		RV=$?
		;;
	10-send-with-imm)
		echo "Starting the client ..."
		echo $CLT $VLD_CCMD $DIR/client $IP_ADDRESS $PORT "1234" "1st_word"
		eval $CLT $VLD_CCMD $DIR/client $IP_ADDRESS $PORT "1234" "1st_word"
		RV=$?
		;;
	11-write-with-imm)
		echo "Starting the client ..."
		echo "$CLT $VLD_CCMD $DIR/client $IP_ADDRESS $PORT "1234""
		eval $CLT $VLD_CCMD $DIR/client $IP_ADDRESS $PORT "1234"
		RV=$?
		;;
	12-receive-completion-queue)
		...
		ROUNDS=3
		echo "Starting the client ..."
		echo "$CLT $VLD_CCMD $DIR/client $IP_ADDRESS $PORT $START_VALUE $ROUNDS"
		eval $CLT $VLD_CCMD $DIR/client $IP_ADDRESS $PORT $START_VALUE $ROUNDS
		RV=$?
		;;
	*)
		echo "Starting the client ..."
		echo "$CLT $VLD_CCMD $DIR/client $IP_ADDRESS $PORT"
		eval $CLT $VLD_CCMD $DIR/client $IP_ADDRESS $PORT
		RV=$?
		;;
	esac

	if [ x"$VLD_CCMD" != x"" ]; then
...
for fault_inject in $(seq 40 40); do
	...
	for srv in $(find $BIN_DIR -name server | sort); do
		...
		if [ $N_SFAILED -gt 0 -o $N_CFAILED -gt 0 ]; then
			echo
			echo "==================================="
			echo "SRV=$SRV"
			echo "CLT=$CLT"
