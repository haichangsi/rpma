...
class IbReadRunner(Runner):
    ...
    def __init__(self, benchmark, config, idfile):
        ...
        ib_name = self._tool + '_' + self._tool_mode
        self.__ib_path = join(self._config.get('IB_PATH', ''), ib_name)
    ...
    def __server_start(self, settings):
        ...
        cfg_r_aux_params = self._config.get('REMOTE_AUX_PARAMS', '').split(' ')
        if cfg_r_aux_params != ['']:
    ...
    def __client_run(self, settings):
        ...
        if self._config.get('DEBUG_SHORT_RUNTIME', False):
            it_opt = '--iters=10'
        else:
            it_opt = '--iters=' + str(settings['iterations'])
        aux_params = [*settings['args']]
        cfg_aux_params = self._config.get('AUX_PARAMS', '').split(' ')
        if cfg_aux_params != ['']:
